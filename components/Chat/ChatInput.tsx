...
export const ChatInput: FC<Props> = ({
  ...
  const handleSend = () => {
    ...
    onSend({ role: "user", content, expandedContent: "[Expanded]" });
    setContent("");
    ...
    <div className="absolute bottom-0 left-0 w-full dark:border-white/20 border-transparent dark:bg-[#444654] dark:bg-gradient-to-t from-[#343541] via-[#343541] to-[#343541]/0 bg-white dark:!bg-transparent dark:bg-vert-dark-gradient pt-6 md:pt-2">
      <div className="stretch mx-2 md:mt-[52px] mt-4 flex flex-row gap-3 last:mb-2 md:mx-4 md:last:mb-6 lg:mx-auto lg:max-w-3xl">
        ...
        <div className="flex flex-col w-full py-2 flex-grow md:py-3 md:pl-4 relative border border-black/10 bg-white dark:border-gray-900/50 dark:text-white dark:bg-[#40414F] rounded-md shadow-[0_0_10px_rgba(0,0,0,0.10)] dark:shadow-[0_0_15px_rgba(0,0,0,0.10)]">
          <textarea
            ref={textareaRef}
            className="w-full p-0 pl-2 m-0 text-black bg-transparent border-0 outline-none resize-none dark:text-white pr-7 focus:ring-0 focus-visible:ring-0 dark:bg-transparent md:pl-0"
            style={{
              resize: "none",
          ...
          <button
            className="absolute p-1 rounded-sm right-5 focus:outline-none text-neutral-800 hover:text-neutral-900 dark:text-neutral-100 dark:hover:text-neutral-200 dark:bg-opacity-50 hover:bg-neutral-200"
            onClick={handleSend}
          >
      ...
      <div className="px-3 pt-2 pb-3 text-xs text-center text-black/50 dark:text-white/50 md:px-4 md:pt-3 md:pb-6">
        <a
          href="https://github.com/cbfrance/aibio002"
          target="_blank"
          rel="noreferrer"
          className="underline"
        >
        ...
        .{" "}
        {t(
          "For research purposes only. Made with GPT-4 by OpenAI and ChatUI by @mckaywrigley"
        )}
      </div>
    </div>
  );
